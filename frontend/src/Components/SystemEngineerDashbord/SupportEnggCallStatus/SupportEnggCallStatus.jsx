...
import "./SupportEnggCallStatus.css";

const SupportEnggCallStatus = () => {
  ...
  const getOutlateData = async () => {
    try {
      ...
      // Filter data based on the logged-in user's ID from localStorage
      const userId = localStorage.getItem("UserID");

      if (response.data.success && response.data.data) {
        const filteredData = response.data.data.filter(
          (item) => item.SupportEnggCallId.UserId === userId
        );

        // Reverse the filtered data array to show the most recent calls first
  ...
  const csvData = [
    ["CallClose User", "CallCloseUser Mobile No","Restaurant Name", "Address", "Restaurant MobileNo", "Call Type", "Call Status", "Date & Time",
     "Support Engg","Call Complete","Date & Time",  "Call Close" , "Date & Time" , "How To Solve Call"]
  ];
    ...
    <>
      ...
      <div className="container mt-5">
        <div className="row">
         ...
         <CSVLink data={csvData} className="btn btn-primary mb-2">Download CSV</CSVLink>
         </div>
   ...
   <p> {selectedItem.UserId.User_Name}</p>
    ...
    <p
                        ...
                        <text
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            handleMobileCall(selectedItem.SupportEnggCallId.SupportEnggMobileNo);
                          }}
                        >
                         Mobile No: {selectedItem.SupportEnggCallId.SupportEnggMobileNo}
                        </text>
          ...
          <div className="">
                  ...
                  <ul className="list-inline items d-flex justify-content-between">
                    ...
                    <li className="list-inline-item items-list">
                      <p
                        ...
                        // style={{ backgroundColor: "#f37a27" }}
                      >
                        {selectedItem.SupportEnggCallId.SupportEnggCall_Status}
                      </p>
                      <br />
              ...
              <hr />
             <div className="m-2"><h6>How To Solve:</h6><p>{selectedItem.SupportEnggCallId.howtosolveinformation}</p></div>
...
export default SupportEnggCallStatus;
